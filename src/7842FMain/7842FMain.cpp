...
void initialize() {
  ...
  display.selector =
    new lib7842::AutonSelector(display.main->newTab("Auton"), {
                                                                {"N", AutonNothing},
                                                                {"c.NrPk", AutonClose_NearPark},
                                                                {"c.NrMd", AutonClose_NearMiddle},
                                                                {"c.NrCr", AutonClose_NearCross},
                                                                {"c.MdPk", AutonClose_MiddlePark},
                                                                {"c.MdNr", AutonClose_MiddleNear},
                                                                {"f.NrPk", AutonFar_NearPark},
                                                                {"f.MdPk", AutonFar_MiddlePark},
                                                                {"f.CrPk", AutonFar_CrossPark},
                                                                {"f.CapNr", AutonFar_CapNear},
                                                                {"f.CapMd", AutonFar_CapMiddle},
                                                                {"f.CapCr", AutonFar_CapCross},
                                                              });
 ...
 * starts.
 */
void competition_initialize() { isCompetition = true; }
...
void opcontrol() {
  ...
  if (isCompetition) { subsystem(flywheel)->setRpm(globalFlywheelRPM); }
  ...
  while (true) {
    ...
    if (remaining < 0_ms) {
      str = std::to_string((int)(opTimer.getDtFromMark().convert(second))) + "  " +
            std::to_string((int)(pros::c::battery_get_capacity())) + "%";
    } else if (remaining > 1_min) {
      str = std::to_string((int)(remaining.convert(minute))) + ":" +
            std::to_string((int)((remaining - 1_min).convert(second))) + "  " +
            std::to_string((int)(pros::c::battery_get_capacity())) + "%";
    } else {
      str = std::to_string((int)(remaining.convert(second))) + "  " +
            std::to_string((int)(pros::c::battery_get_capacity())) + "%";
    }
...
void autonomous() {
  ...
  // create timer
  AutonPasser passer =
    std::make_tuple(SideController(robot.chassis, display.selector->getSelectedSide()), Timer());
